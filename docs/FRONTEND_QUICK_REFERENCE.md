...
Response: { "success": true, "message": "文件已删除" }
...
POST /api/v1/tasks
{
  "audio_files": ["uploads/user_123/meeting.wav"],  // 使用上传后的路径
  "file_order": [0],
...
// 示例
GET /api/v1/tasks?state=running&limit=20  // 只获取进行中的任务
GET /api/v1/tasks?state=success&limit=50  // 只获取已完成的任务
```
...
```typescript
// 列出模板
GET /api/v1/prompt-templates?scope=global&user_id=...

// 获取详情
GET /api/v1/prompt-templates/{template_id}?user_id=...

// 创建私有模板
POST /api/v1/prompt-templates?user_id=...

// 更新/删除
PUT/DELETE /api/v1/prompt-templates/{template_id}?user_id=...
```
