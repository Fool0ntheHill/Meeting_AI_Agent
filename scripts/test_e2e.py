...
使用方法:
    python scripts/test_e2e.py --audio meeting.wav
    python scripts/test_e2e.py --audio meeting.wav --skip-speaker-recognition
    python scripts/test_e2e.py --audio part1.wav part2.wav --file-order 0 1
"""
...
async def test_pipeline(
    ...
    try:
        ...
        logger.error(f"✗ 管线处理失败: {e}", exc_info=True)
        raise
...
示例:
  ...
  # 指定会议描述
  python scripts/test_e2e.py --audio meeting.wav --description "产品规划会议"
        """,
    )
    ...
    parser.add_argument(
        ...
        help="配置文件路径(默认: config/development.yaml)",
    )
